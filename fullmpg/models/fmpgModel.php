...
class fmpgModel {
	...
	public function getUser($name, $pass) {// start getUser
		$stmt = $this->db->prepare("
			SELECT id, user_name, user_password, email, user_salt, user_fullname
			FROM users 
	...
	public function getFill() {//getfill
	$statement = $this->db->prepare("
		SELECT id, user_name, user_fullname, user_created
		FROM users
	...
	public function addUser($user_name, $user_fullname, $email, $user_password, $user_salt, $user_created) {
		...
		try {
			if ($query->execute()) {
				echo "<p class='useradded'>Added an account for <em>$user_fullname</em>. (<a href='auth.php'>Log in</a>)<br /><br /></p>";
			}
		} 
		catch (\PDOException $e) {
